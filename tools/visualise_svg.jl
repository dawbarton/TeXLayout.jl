...
# ── SVG helpers ────────────────────────────────────────────────────────────────
svg_rect(io, x, y, w, h; fill, stroke = "none", sw = 1, dash = "") = println(
    io,
...
svg_line(io, x1, y1, x2, y2; stroke, sw = 1, dash = "") = println(
    io,
...
svg_text(io, x, y, s; size = 10, fill = "#333") = println(
    io,
...
function main()
    ...
    open(outf, "w") do io
        ...
        println(
            io, """<svg xmlns="http://www.w3.org/2000/svg" width="$W" height="$H" """ *
                """viewBox="0 0 $W $H">"""
        )
        ...
        svg_line(
            io, 0, cy(axis_em), W, cy(axis_em);
            stroke = "#cc0000", sw = 1, dash = "6,4"
        )
        svg_line(
            io, 0, cy(0.0), W, cy(0.0);
